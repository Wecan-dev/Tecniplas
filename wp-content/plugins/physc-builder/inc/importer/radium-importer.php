...
if ( !class_exists( 'Radium_Theme_Importer' ) ) {
	class Radium_Theme_Importer {
		...
		public function set_demo_theme_options( $file ) {
			...
			if ( !file_exists( $file ) ) {
				wp_die(
					__( 'Theme options Import file could not be found. Please try again.', 'physc-builder' ),
					'',
			...
			do_action( 'demo_importer_after_theme_options_import', $this->demo_files_path );
		...
		public function process_setting_import_file( $file ) {
			...
			foreach ( $keys as $key ) {
				if ( $option_data = unserialize( $woo_datas[$key] ) ) {
					...
					$format = esc_html__( 'Update Option "%d" error', 'physc-builder' );
		...
		function process_widget_import_file( $file ) {
			...
			if ( !file_exists( $file ) ) {
				wp_die(
					__( 'Widget Import file could not be found. Please try again.', 'physc-builder' ),
					'',
					array( 'back_link' => true )
				);
			}
		...
		public function import_widgets( $data ) {
			...
			foreach ( $data as $sidebar_id => $widgets ) {
				...
				if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
					...
					$sidebar_message      = __( 'Sidebar does not exist in theme (using Inactive)', 'physc-builder' );
				}
				// Result for sidebar
				...
				foreach ( $widgets as $widget_instance_id => $widget ) {
					...
					if ( !$fail && !isset( $available_widgets[$id_base] ) ) {
						...
						$widget_message      = __( 'Site does not support widget', 'physc-builder' ); // explain why widget not imported
					}
					...
					if ( !$fail && isset( $widget_instances[$id_base] ) ) {
						...
						foreach ( $single_widget_instances as $check_id => $check_widget ) {
							...
							if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
								...
								$widget_message      = __( 'Widget already exists', 'physc-builder' ); // explain why widget not imported
								break;
							}
						}
					}
					...
					if ( !$fail ) {
						...
						if ( $sidebar_available ) {
							...
							$widget_message      = __( 'Imported', 'physc-builder' );
						} else {
							$widget_message_type = 'warning';
							$widget_message      = __( 'Imported to Inactive', 'physc-builder' );
						}
					}
					// Result for widget instance
					$results[$sidebar_id]['widgets'][$widget_instance_id]['name']         = isset( $available_widgets[$id_base]['name'] ) ? $available_widgets[$id_base]['name'] : $id_base; // widget name or ID if name not available (not supported by site)
					$results[$sidebar_id]['widgets'][$widget_instance_id]['title']        = $widget->title ? $widget->title : __( 'No Title', 'physc-builder' ); // show "No Title" if widget instance is untitled
