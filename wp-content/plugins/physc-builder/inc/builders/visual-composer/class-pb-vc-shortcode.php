...
if ( ! class_exists( 'PhyscBuilder_VC_Shortcode' ) ) {
	/**
	 * Class PhyscBuilder_VC_Shortcode
	 */
	class PhyscBuilder_VC_Shortcode {
		/**
		 * @var string
		...
		/**
		 * PhyscBuilder_Abstract_Shortcode constructor.
		 */
		public function __construct() {
			...
			add_shortcode( 'physc-' . $this->base, array( $this, 'shortcode' ) );
		...
		public function setup_shortcode() {
			...
			$icon = file_exists( $this->assets_path . 'images/icon.png' ) ? $this->assets_url . 'images/icon.png' : PHYSC_BUILDER_URL . 'assets/images/icon.png';

			vc_map(
				array(
					'name'        => $this->name,
					'base'        => 'physc-' . $this->base,
					'category'    => __( 'Physc Builder', 'physc-builder' ),
		...
		public function shortcode( $atts ) {
			...
			$atts = apply_filters( "physc-builder/vc/$this->base/shorcode_attrs", $this->_handle_attrs( $atts ) );
		...
		public function render( $atts ) {
			...
			do_action( 'physc-builder/before-modules-template', $this->base );

			ob_start();
			physc_builder_get_template( $this->base, array( 'params' => $atts ), $atts['template_path'] );
			$html = ob_get_clean();
